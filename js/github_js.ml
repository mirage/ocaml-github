...
module Cohttp_lwt_xhr  = struct
    ...
    module Client = struct
        (*val call : ?headers:Cohttp.Header.t -> ?body:CLB.t -> ?chunked:bool -> Cohttp.Code.meth -> Uri.t -> 
            (Response.t * CLB.t) Lwt.t *)
        ...
        (* val post : ?body:CLB.t -> ?chunked:bool -> ?headers:Cohttp.Header.t -> Uri.t -> 
            (Response.t * CLB.t) Lwt.t *)
        let post ?body ?chunked ?headers uri = Lwt.fail Not_found

    end
end

module Github' = Github_core.Make(Cohttp_lwt_xhr)
include Github'
